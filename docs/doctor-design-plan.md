...
7. **Done.** Score layer — `Report.ComputeScore` behind `doctor --report --score` (full-tree runs only): smooth uncalibrated decay over severity-weighted finding counts, presentation-only per decision 4b; nothing gates on it and cross-repo comparison is documented as meaningless until calibrated.
