// Package astcache holds the long-lived index caches (Phase 2 of
// docs/mcp-server-plan.md) plus the by-name call-graph index they populate.
//
...
//     Shared by the call-graph builder AND the find-* analyzers (via SeedASTs),
//     so a find-callers call reuses ASTs a prior callgraph call already parsed.
//   - CallIndexCache: path -> per-file defs + raw calls. Lets BuildCallIndex
//     skip the AST walk for unchanged files and only re-run the (cheap)
...
// the tree each call, so added/removed files are always seen; the caches only
// memoize the expensive per-file parse/extract step.
package astcache
...
// NewParseCache returns an empty parse cache with a fresh FileSet.
...
// or parsed are skipped (best-effort).
