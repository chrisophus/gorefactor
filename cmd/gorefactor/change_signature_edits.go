...
// textEdit is a byte-range replacement in a file. Offsets come from the
// shared token.FileSet of a packages.Load run.
...
func editFiles(edits []textEdit) []string {
...
// applyTextEdits applies edits grouped per file (descending offset so earlier
// offsets stay valid), parse-checks every result before writing, then runs
// goimports on each touched file.
func applyTextEdits(edits []textEdit) error {
	...
	for _, file := range editFiles(edits) {
		list := byFile[file]
...
// paramTexts renders flattened params as "name type" strings. forceNamed
// names unnamed params "_" so a named param can join the list legally.
...
func buildSignatureEdits(pkgs []*packages.Package, tgt *sigTarget, action *sigAction, locator string) ([]textEdit, string, error) {
	...
	return nil, "", usageErrorf("change-signature: unknown action %q", action.kind)
}
...
// declared at defPos and returns one edit per use (or just the positions
...
func buildRenameParamEdits(tgt *sigTarget, params []sigParam, action *sigAction, locator string) ([]textEdit, string, error) {
	var target *sigParam
	for i := range params {
		...
		if params[i].name == action.newName {
			return nil, "", usageErrorf("parameter %q already exists on %s", action.newName, locator)
		}
	}
	if target == nil {
	...
	if rn := receiverName(tgt.fn); rn != "" && rn == action.newName {
		return nil, "", usageErrorf("parameter name %q collides with the receiver name", action.newName)
	}
...
func resolveParamRef(params []sigParam, ref, locator string) (int, error) {
	if n, err := strconv.Atoi(ref); err == nil {
		if n < 0 || n >= len(params) {
			return -1, notFoundErrorf("parameter index %d out of range on %s (0-%d)", n, locator, len(params)-1)
		}
		return n, nil
	}
	for i, p := range params {
...
func callValueFor(tgt *sigTarget, action *sigAction) (string, error) {
	if action.callValue != "" {
	...
	v := zeroValueExpr(typ, action.paramType)
	if v == "" {
		return "", usageErrorf("cannot infer a zero value for type %s; pass --call-value EXPR", action.paramType)
	}
	return v, nil
}
