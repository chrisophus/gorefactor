...
- **Sensors** (feedback): `lint` aggregates 41 structural rules (size, duplication, design smells, error handling, coverage, dead-code, arch) and (where safe) autofixes `file-size` / `dead-code` / `error-not-wrapped` via `--fix`. Run it as a final gate after a refactor batch — anything not under control surfaces here.
...
- `lint [path] [--fix [--verify] | --probe-fixes] [--baseline | --write-baseline] [--baseline-file PATH] [--json] [--max N] [--fail-only] [--info] [--verbose]`: Structural linter, 41 default rules (canonical list in `cmd/gorefactor/lint_registry_test.go`). By default `[info]` issues (e.g. `high-blast-radius`, `untested-*`) are hidden so actionable warnings aren't buried; `--info` shows them (collapsing per-file `high-blast-radius` into one summary line) and `--verbose` shows everything uncollapsed. A `lint.duplicate-ignore` list in `.gorefactor.yaml` excludes extra normalized-code patterns from `duplicate-block` (canonical error idioms like `if err != nil { return err }` are already excluded built-in).
  ...
  - *harness residue* (born from a code review of this repo's own autofix scars): `generated-name` (a committed function still named `extractBlockL<line>` — the extractor's positional fallback; re-inline or rename), `byvalue-buffer` (a `bytes.Buffer`/`strings.Builder` parameter passed by value — writes inside the callee are silently lost to the caller; this exact bug shipped in the add-test scaffold generator), `stranded-comment` (a doc comment whose leading identifier names a *different* top-level declaration in the package than the one it precedes — the orchestrator/types.go failure mode where mechanical edits moved declarations out from under their comments; non-test files only, since test doc comments conventionally open with the tested symbol's name), `orphaned-config-path` (a `.golangci.yml` path regex or lint-baseline file entry that matches nothing in the tree — a rename un-exempts silently, so orphaned exemptions surface in the same commit; the scan walks the whole tree, not lint's Go-file walk, so fixture-dir exemptions aren't false orphans)
  ...
  - **Autofix outcome journal** (`.gorefactor/autofix-outcomes.jsonl`): every attempt's result — applied / reverted-by-gate / no-target — is journaled per finding fingerprint (passive, never fails a run). The fixer skips fixes the gate already reverted instead of paying a build+test cycle to rediscover a recorded fact (delete the journal to retry). Lint folds outcomes back into findings through a `note` field that is excluded from fingerprints, so baseline and journal keys stay stable: a gate-reverted `dead-code` deletion **demotes the finding to info** — the revert is execution proof the symbol is reachable by means the name-based scan cannot see; a reverted `error-not-wrapped` keeps its severity but notes the error text may be contractual (a test probably matches it); a no-target extraction on `long-function`/`complexity`/`extract-candidate` replaces the implied "consider extracting" with "needs restructuring". Execution results outrank static analysis — this journal is how the sensors learn from the gate. Two ways the signal is gathered, split by cost: the **no-target check runs live inside lint** (whether the reducer can offer a nameable, non-vacuous block is pure single-file analysis, so size findings the fixer can't help carry the needs-restructuring note without any fix run), while **`--probe-fixes` buys the execution-grade signal** — it runs the full verify pipeline (apply, batch, gate, bisect) but *always restores the tree*, journaling `verified` (gate passed; the finding gains an "autofix verified safe" note) or `reverted` per fix. Probe is mutually exclusive with `--fix`, accepts `--fix-level aggressive`, and costs the same build+test cycles as a verified fix run — a pure sensor pass for deciding what `--fix` would actually do.
...
- Refine and iterate (up to 40 iterations by default)
...
# Iteration control
-max-iter N                        # Max iterations (0 = mode default: 40 for agentic, 3 for single-shot)
