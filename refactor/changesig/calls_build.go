...
func buildAddParamEdits(pkgs []*packages.Package, tgt *sigTarget, params []sigParam, action *Action, locator string) ([]TextEdit, string, error) {
	for _, p := range params {
		if p.name == action.ParamName {
			return nil, "", cerr.Usagef("parameter %q already exists on %s", action.ParamName, locator)
		}
	}
	if rn := receiverName(tgt.fn); rn != "" && rn == action.ParamName {
		return nil, "", cerr.Usagef("parameter name %q collides with the receiver name", action.ParamName)
	}
	arity := len(params)
	...
	if idx == -1 {
		if variadicIndex >= 0 {
			return nil, "", cerr.Usagef("%s is variadic; pass --position N with N <= %d (before the variadic parameter)", locator, variadicIndex)
		}
		idx = arity
	}
	if idx > arity {
		return nil, "", cerr.Usagef("--position %d out of range (0-%d)", idx, arity)
	}
	if variadicIndex >= 0 && idx > variadicIndex {
		return nil, "", cerr.Usagef("--position %d would place the parameter after the variadic parameter (max %d)", idx, variadicIndex)
	}
...
func buildRemoveParamEdits(pkgs []*packages.Package, tgt *sigTarget, params []sigParam, action *Action, locator string) ([]TextEdit, string, error) {
	...
	target := params[idx]
	if target.variadic {
		return nil, "", cerr.Usagef("removing the variadic parameter is not supported")
	}
