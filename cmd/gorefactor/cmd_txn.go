...
// txnCommand reads gorefactor mutation commands (one per line, `#` comments)
// from a file or stdin and applies them in-process as a single transaction:
// any failure restores every touched file and reports the failing line; on
// success the whole batch is journaled as one undoable unit.
func txnCommand(args []string) error {
	...
	for _, f := range collector.touched() {
		...
		if _, err := os.Stat(f); err != nil {
			continue // deleted by a later op
		}
