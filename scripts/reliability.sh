...
lines.append("- **success** = task done AND `go build`+`go test` green (gate is ground "
             ...
             "- **mean secs** = wall-clock per run. The junior is free in frontier tokens "
             "but not in time: if it is too slow the senior won't delegate to it, so this "
             "is the real adoption gate, tracked alongside tokens.\n"
