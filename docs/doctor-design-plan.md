...
Status: purified via AISP 5.1 (source: `gorefactor-doctor-v2.aisp`); revised 2026-07-17 after codebase-integration review. **All build order steps (1–7) are implemented**; see per-step status in Build order. Remaining work is empirical (Open items): observing rule false-positive profiles on real usage, the golangci scoped-latency measurement, and graduation-threshold tuning from the findings journal.
Date: 2026-07-16
...
| String `+=` in loops → `strings.Builder` | string concat in hot paths | perf | **Implemented** (`string-concat-in-loop`, warning; provably-string RHS only) |
| Linear search inside a loop → build a map | use Set/Map for repeated lookups | perf | **Implemented** (`linear-search-in-loop`, info — the rule cannot see n and small-slice scans dominated observation on this repo) |
| Naked goroutines / `NewTicker` without `Stop` in library code | `effect-needs-cleanup` | conc | **Implemented** (`unstopped-ticker` warning; `naked-goroutine` info, lifecycle-ident heuristic; both exempt package main) |
| Context misuse (`Background()`/`TODO()` where a ctx is in scope; loops without `ctx.Done()`) | lifecycle wiring family | conc | **Implemented** reuse-first: golangci's `contextcheck`/`containedctx` enabled in config, mapped to conc by the golangci substrate |
| Pass-through parameters (forwarded ≥3 call layers unused) | prop drilling | struct | **Implemented** (`pass-through-param`, info; name-based, package-local, shadowing bails out) |
| `panic`/`log.Fatal` in library packages | server/client boundary violations | struct, shape-conditioned | **Implemented** (`fatal-in-library`: log.Fatal*/os.Exit warn, panic info; package main and tests exempt) |
| Dependency hygiene (`go mod tidy -diff` substrate; heavyweight single-symbol imports) | bundle size / barrel imports | dead | **Implemented** (`modtidy` substrate: untidy findings with FixCmd, single-use direct deps as info; a true "heavyweight" signal needs module size data — deferred) |
...
4. **Done.** govulncheck + deadcode (full-run tier) + shape detection — `doctor.Govulncheck` (symbol-level/reachable findings only, sec/error, gating; unavailable-never-silent when the vuln DB or binary can't run), `doctor.Deadcode` (x/tools deadcode `-json -test`, dead/warning), `doctor.DetectShape` (module path, Go version, main dirs vs library, Temporal/Kafka presence), plus `doctor.ModTidy` (`go mod tidy -diff` with a FixCmd; info-severity single-use-dependency hints — the "heavyweight single-symbol import" half is deferred until a size signal exists) and the shape-conditioned `fatal-in-library` structural rule.
5. **Done.** Temporal custom analyzers — evaluated wrapping Temporal's official `workflowcheck` analyzer per the reuse-first note: `doctor.Temporal` runs the `workflowcheck` binary when on PATH and falls back to an in-process parse-only scan of `workflow.Context` functions otherwise (importing workflowcheck as a library was rejected — it would drag go.temporal.io/sdk into every build). Rules: temporal/time, temporal/rand, temporal/goroutine, temporal/select, temporal/channel; tmprl/error, gating, trivially green without the SDK dependency. The rest of the step-5 inventory landed as structural rules (`string-concat-in-loop`, `linear-search-in-loop`, `unstopped-ticker`, `naked-goroutine`, `pass-through-param`) and context misuse went reuse-first via golangci's `contextcheck`/`containedctx` + conc category mapping.
6. **Done.** Agent-install mode — `gorefactor doctor install [--target claude.md|cursor|agents.md|all]` emits rule expectations into agent context, generated from the live rule registry (idempotent, sentinel-marked). FixCmd mapping completed: every exported finding's FixCmd is runnable verbatim (internal dispatcher-shaped autofix commands gain the binary prefix at the substrate boundary).
7. **Done.** Score layer — `Report.ComputeScore` behind `doctor --report --score` (full-tree runs only): smooth uncalibrated decay over severity-weighted finding counts, presentation-only per decision 4b; nothing gates on it and cross-repo comparison is documented as meaningless until calibrated. First calibration decision applied: ranking-signal rules (`high-blast-radius`, `low-gorefactor-adherence`) are score-exempt — they describe context/process, not defects, and were drowning the signal (731 of 902 findings on this repo). Second calibration decision (value tiers): weight per rule by how reliably fixing its findings is a genuine improvement, not by severity alone. Defect rules (duplication, error handling, dead code, perf, gate integrity, lifecycle) keep full severity weight; size/shape proxy rules (`long-function`, `complexity`, `god-object`, `excessive-params`, …) count half — their finding counts can be lowered by pure code motion (wrap the body in one helper, bundle params into a struct unpacked on the next line), and halving the weight halves the payoff of that churn while keeping real pressure; convention rules (`funcorder-*`, autofix-enforced style) count zero; `untested-package` is overridden to warning-equivalent weight because a package with no tests is an actionable testability defect whose fix — writing the first test — cannot be faked (`untested-function` stays exempt as review-granularity noise). Classification typos are caught by a registry cross-check test (`ScoreClassifiedRules`).
