...
func TestListSnapshots_MissingBaseDir(t *testing.T) {
	withWorkDir(t, t.TempDir())

	dirs, err := ListSnapshots()
	if err != nil {
		t.Fatalf("expected nil error on missing snapshots dir, got: %v", err)
	}
	if len(dirs) != 0 {
...
func TestListSnapshots_ReturnsSortedDirectories(t *testing.T) {
	dir := t.TempDir()
	withWorkDir(t, dir)

	snapsDir := filepath.Join(".gorefactor", "snapshots")
	for _, name := range []string{"gamma", "alpha", "beta"} {
		if err := os.MkdirAll(filepath.Join(snapsDir, name), 0755); err != nil {
			t.Fatal(err)
		}
	}
	...
	if dirs[0] != filepath.Join(snapsDir, "alpha") {
		t.Errorf("expected alpha first, got %s", dirs[0])
	}
	if dirs[2] != filepath.Join(snapsDir, "gamma") {
		t.Errorf("expected gamma last, got %s", dirs[2])
	}
}

// ---- RestoreSnapshot ----

func TestRestoreSnapshot_RestoresFileToAbsolutePath(t *testing.T) {
	tmp := t.TempDir()

	// Create the target file with "current" content.
	targetFile := filepath.Join(tmp, "restore_target.go")
	if err := os.WriteFile(targetFile, []byte("package main\n// current\n"), 0644); err != nil {
		t.Fatal(err)
	}

	// Build a snapshot directory that mirrors the absolute file path.
	// RestoreSnapshot prepends filepath.Separator to the relative path inside
	// the snapshot dir, so the mirror path is snapDir + absolute-path-without-root.
	snapDir := filepath.Join(tmp, "snapshot")
	// targetFile is absolute, e.g. /tmp/xxx/restore_target.go
	// Inside the snapshot dir it lives at snapDir + targetFile[1:]
	snapMirror := filepath.Join(snapDir, targetFile[1:])
	if err := os.MkdirAll(filepath.Dir(snapMirror), 0755); err != nil {
		t.Fatal(err)
	}
	originalContent := []byte("package main\n// original\n")
	if err := os.WriteFile(snapMirror, originalContent, 0644); err != nil {
		t.Fatal(err)
	}

	count, err := RestoreSnapshot(snapDir)
	if err != nil {
		t.Fatalf("RestoreSnapshot: %v", err)
	}
	if count != 1 {
		t.Errorf("expected 1 file restored, got %d", count)
	}
...
func TestSnapshotDir_ReturnsExpectedPath(t *testing.T) {
	got := SnapshotDir("my-plan")
	want := filepath.Join(".gorefactor", "snapshots", "my-plan")
	if got != want {
