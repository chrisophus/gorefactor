...
func init() {
	...
	registerCommand(Command{
		...
		Usage:       "lint [path] [--fix] [--verify] [--fix-level safe|aggressive] [--baseline] [--no-baseline] [--write-baseline] [--baseline-file PATH] [--baseline-ratchet REF] [--json] [--sarif] [--quiet] [--fail-only] [--info] [--verbose] [--max N] [--rule NAME] [--skip-rule NAME] [--fail-on error|warning] [--config PATH] [--profile NAME]",
		MinArgs:     0,
		MaxArgs:     1,
		Flags: map[string]bool{
			"--baseline":         false,
